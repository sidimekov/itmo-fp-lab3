...
Для каждого алгоритма поддерживается собственное состояние. Состояние передается между шагами обработки как аргумент рекурсивной функции, без использования мутабельных переменных. Это позволяет корректно обрабатывать поток данных по шагам

Потоковая обработка реализована через явное хранение и обновление состояния. При поступлении новой точки состояние обновляется и, если возможно, вычисляются новые интерполированные значения. При завершении входного потока производится финальный вывод оставшихся данных
