...
Линейная интерполяция использует две соседние точки и аппроксимирует функцию линейным отрезком между ними. Для каждого значения x на равномерной сетке вычисляется соответствующее значение y.

Интерполяция Ньютона строит интерполяционный многочлен по набору из n точек. В потоковом режиме используется скользящее окно из последних n точек. Для вычисления значений выбирается центральная часть окна, так как в ней интерполяция наиболее устойчива.
...
Модуль `engine.ml` определяет общий интерфейс для алгоритмов интерполяции. В нем описаны типы конфигурации, генерация равномерной сетки по оси x и функции, которые применяют выбранные алгоритмы к набору точек. Этот модуль используется связывает вычисления и внешний код

Файл `my_lab3.ml` содержит логику работы с вводом и выводом, там реализован разбор аргументов командной строки, чтение входных данных из stdin и печать результатов в stdout. Для каждого алгоритма поддерживается собственное состояние - это позволяет обрабатывать поток данных по шагам

Потоковая обработка реализована через явное хранение состояния. При поступлении новой точки состояние обновляется и, если возможно, вычисляются новые интерполированные значения. При завершении входного потока производится финальный вывод оставшихся данных.
